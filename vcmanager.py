...
from Eaglebot.core.data import _sudousers_list, _vcusers_list
from Eaglebot.core.managers import edit_delete, edit_or_reply
from Eaglebot.helpers.utils import mentionuser
from Eaglebot.sql_helper import global_collectionjson as sql
...
async def start_vc(event):
    ...
    vc_chat = await eagle.get_entity(event.chat_id)
...
async def end_vc(event):
    ...
    if not gc_call:
        return
    try:
        await eagle(functions.phone.DiscardGroupCallRequest(call=gc_call))
...
async def title_vc(event):
    ...
    await eagle(functions.phone.EditGroupCallTitleRequest(call=gc_call, title=title))
    ...
    if cmd == "get":
        if not vc_chats:
            return await edit_delete(
                event, "__There are no vc auth users for your Eaglebot.__"
            )
        result = "**The list of vc auth users for your Eaglebot are :**\n\n"
        for chat in [*vcusers]:
        ...
        if replied_user is None:
            return
        replied_user = await eagle.get_entity(replied_user)
