...
from Eaglebot.core import check_owner

from .function import vc_player

vcimg = "https://github.com/badmunda98/EagleVc/blob/Bad/resources/vcfileW.mp4"
...
# MAINMENU BUTTONS
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^joinvc$")))
@check_owner(vc=True)
async def joinvc(event):
    ...
    try:
        vc_chat = await eagle.get_entity(chat)
...
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^leavevc$")))
@check_owner(vc=True)
...
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^playervc$")))
@check_owner(vc=True)
...
# PLAYER BUTTONS
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^menuvc$")))
@check_owner(vc=True)
async def playervc(event):
    await event.edit(file=vcimg, text="**| VC Menu |**", buttons=buttons[0])


@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^previousvc$")))
@check_owner
...
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^resumevc$")))
@check_owner(vc=True)
...
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^pausevc$")))
@check_owner(vc=True)
...
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^skipvc$")))
@check_owner(vc=True)
...
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^repeatvc$")))
@check_owner(vc=True)
...
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^settingvc$")))
@check_owner
...
@eagle.tgbot.on(CallbackQuery(pattern="^(a|b|c)mode$"))
@check_owner
async def vc(event):
...
@eagle.tgbot.on(CallbackQuery(pattern="(a|b|c)modeinfo$"))
@check_owner(vc=True)
async def vc(event):
    mode = (event.pattern_match.group(1)).decode("UTF-8")
    if mode == "a":
        text = "⁉️ What is This?\n\n🏢 Public: Anyone can use Eaglebot vc player present in this group.\n\n🏠 Private: Only Owner of user bot and sudo users can use Eaglebot vc player"
    elif mode == "b":
...
# COMMON BUTTONS
@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^backvc$")))
@check_owner(vc=True)
async def vc(event):
    await event.edit("** | VC Menu | **", buttons=buttons[0])


@eagle.tgbot.on(CallbackQuery(data=re.compile(r"^vc_close$")))
@check_owner(vc=True)
async def vc(event):
    try:
        await event.delete()
