...
from userbot.helpers import _eagleutils, fileinfo
...
class EagleVc:
    ...
    async def join_vc(self, chat, join_as=None):
        ...
        try:
            await self.app.join_group_call(
                chat_id=chat.id,
                stream=AudioPiped("EagleVc/resources/Silence01s.mp3"),
    ...
    async def leave_vc(self):
        ...
        await _eagleutils.runcmd("rm -rf temp")
        self.clear_vars()
        ...
        if not self.PLAYLIST:
            if self.PLAYING:
                await self.app.change_stream(
                    self.CHAT_ID,
                    AudioPiped("EagleVc/resources/Silence01s.mp3"),
                )
            self.PLAYING = False
